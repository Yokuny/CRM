...
- **Phase / Task**: Specify → Design → Tasks (aprovadas, commit `d6df6da` em `main`) → **Execute em andamento** (Lote 5/6 completo, Lote 6/6 — o último — a disparar).
- **Completed**:
  ...
  - Lote 4 (Fase 6+7, T25-T32 — env config, `metaClient`, webhook, 3 workers, wiring do `ai-gateway`). 9 commits (`bc2e3b4`..`c8add9d`, incl. 2 auto-correções do worker). Rodou limpo, sem interrupções.
  - Lote 5 (Fase 8+9, T33-T40 — `apps/crm-api`: `Channel` repository/service/controller/router, `Conversation` takeover/release/envio manual + router). 9 commits (`d7c058d`..`e8de901`, incl. 1 auto-correção de formatação `1f55b5a`). Edições consequenciais necessárias (exigidas pelo próprio Done-when de T34/T40, não scope creep): `CHANNEL_ENC_KEY` adicionada a `apps/crm-api/src/config/env.config.ts` + valor real em `.env.example` (T34 precisa cifrar/mascarar o token do Channel; só `ai-gateway` tinha essa var antes); `"@crm/ai-kit": "workspace:*"` adicionada a `apps/crm-api/package.json` (T40 precisa chamar o `runTurn` REAL para provar que `mode:'human'` bloqueia o loop do bot — teste em `conversation.router.e2e.test.ts` importa `runTurn` de `@crm/ai-kit` e espiona o client Anthropic fake, confirma `runLoop` nunca chamado). Rodou limpo, sem interrupções; verificado de novo de forma independente pelo orquestrador (repository/e2e revisados manualmente: guard da janela de 24h rejeita ANTES do insert, `type:'text'` para template também é correto — o schema de `Message` não tem um `type:'template'` dedicado, `templateName`/`templateLanguage`/`templateParams` são os campos que já existiam desde T4).
  - 704 testes passando (full suite, `pnpm vitest run`), `tsc --noEmit` limpo, `biome check .` limpo (mesmos 2 itens pré-existentes fora de escopo, ainda não tocados).
- **In-progress**: nenhum (Lote 5 fechado e commitado; Lote 6 — o último — ainda não disparado nesta escrita do Handoff).
- **Next step**: disparar Lote 6 (Fase 10+11, T41-T47 — wiring final do `app.ts` do `crm-api`, estender isolamento entre tenants, golden set (`evals/`), script `pnpm run evals`, P2 áudio transcrito via Whisper). **Este é o ÚLTIMO lote** — ao final dele, TODAS as 48 tasks (47 originais + T24B) estarão commitadas, e o Verifier deve ser disparado automaticamente (author ≠ verifier), sem precisar ser pedido de novo, conforme o Execution Protocol do `tasks.md`.
- **Blockers**: nenhum. Notas operacionais:
  1. Lotes 1-3 sofreram interrupções de infraestrutura (sleeps/stalls/turnos prematuros), todas retomadas sem perda de trabalho real; Lotes 4 e 5 rodaram limpos, sem nenhuma interrupção.
