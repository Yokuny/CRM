...
- **Feature**: `ai-gateway` (feature 5 de 11) — **Execute em andamento**. `crm-web-shell` (feature 4) já está mergeada em `main` (PR #3, `076cfe5`).
- **Phase / Task**: Specify → Design → Tasks (aprovadas, commit `d6df6da` em `main`) → **Execute completo (48/48 tasks)** → **Verifier a disparar**.
- **Completed**:
  - Lote 1 (Fase 1+2, T1-T9 — `packages/db` models + crypto + `packages/contracts` schemas). 9 commits (`d1f36d2`..`9d5f792`) + 3 correções do orquestrador (`02da05a`, `5d16761`, `d611a08`).
  - Lote 2 (Fase 3+4, T10-T17 — scaffold `@crm/ai-kit`, providers, `TOOL_DEFINITIONS`, os 4 tool executors do Anel A). 8 commits (`a6bc221`..`009fdc8`) + 1 correção (`d1791bb`).
  - Lote 3 (Fase 5, T18-T24 — pipeline `ingest→...→runTurn`). 7 commits (`54a39b9`..`0ec2ba8`) + **T24B** (`04d6f80`, bug de `turnLock` nunca liberado em `human_mode`/`guard_rejected` + `fixedReply` nunca entregue — corrigido, documentado em `tasks.md`).
  - Lote 4 (Fase 6+7, T25-T32 — env config, `metaClient`, webhook, 3 workers, wiring do `ai-gateway`). 9 commits (`bc2e3b4`..`c8add9d`, incl. 2 auto-correções do worker). Rodou limpo.
  - Lote 5 (Fase 8+9, T33-T40 — `apps/crm-api`: `Channel`/`Conversation` repository/service/controller/router). 9 commits (`d7c058d`..`e8de901`, incl. 1 auto-correção `1f55b5a`). Edições consequenciais: `CHANNEL_ENC_KEY` em `apps/crm-api/src/config/env.config.ts`+`.env.example`; `"@crm/ai-kit"` adicionada a `apps/crm-api/package.json` (T40 chama o `runTurn` REAL para provar que takeover bloqueia o bot).
  - Lote 6 — **ÚLTIMO** (Fase 10+11, T41-T47 — wiring final do `crm-api`, isolamento entre tenants estendido, golden set (`evals/`), script `pnpm run evals`, P2 áudio via Whisper). 7 commits (`5862dff`..`9bd8255`). T43 implementou de fato o glob `evals/**/*.int.test.ts` em `vitest.config.ts` (a nota pré-lote dizia "já feito" — na verdade só o texto do `tasks.md` tinha sido escrito antes do Execute, o código em si ficou para T43 mesmo, como o próprio Done-when já prescrevia) + `@crm/db` como devDependency raiz (evals/ vive fora de qualquer package). T47 resolveu a dependência cruzada `packages/ai-kit`→`metaClient` (que vive em `apps/ai-gateway`) via DI: `ingest.ts` declara só o TIPO `DownloadAudio`, a implementação real fica em `apps/ai-gateway/src/providers/audioDownloader.ts`, injetada via `webhook.router.ts` — mesmo molde do `AnthropicClient`/`WhisperClient`. Sofreu 1 interrupção por rate-limit de sessão (madrugada, resolvida sozinha) + 4 stalls de stream, todos retomados sem perda de trabalho; corrigi o worker uma vez para NÃO editar `.specs/lessons.json` (arquivo machine-owned da skill, erro de formatação ali é baseline pré-existente desde antes desta feature, aceito em todo Build gate do Execute).
  - **728 testes passando** (full suite, `pnpm vitest run`), `tsc --noEmit` limpo, `biome check .` limpo (mesmos 2 itens pré-existentes fora de escopo, nunca tocados: `noExplicitAny` em `apps/web` de `crm-web-shell`, formatação de `.specs/lessons.json`). Verificado de forma independente pelo orquestrador ao final de cada lote, não só pelo relato do próprio worker.
  - **Verifier iteração 1** (`9fefced docs(ai-gateway): record Verifier FAIL (iteration 1)`): **FAIL** — 37/48 ACs verificados limpos, 4 spec-precision gaps (já "não confirmado" no próprio spec.md — não falha de implementação), **7 gaps reais**: AIG-11 (aviso de rate-limit sem throttle de 1/janela), AIG-44 (nenhum dos 5 logs estruturados exigidos existia), AIG-48 (figurinha/vídeo descartados, `caption` nunca populado), AIG-38/AIG-41 (gaps de cobertura de teste, comportamento provavelmente correto), AIG-08 (metade do log "canal não resolvido" faltando, dobrado em AIG-44), AIG-43 (nenhuma feature do projeto tem CI — sistêmico). Sensor de mutação: 6/6 mutações mortas (tier P0, isolamento entre tenants/turnLock/janela 24h/dedup/cripto todos sólidos sob fault injection real).
  - **AD-031** (`c2e3468`): CI via GitHub Actions rodando o script `check` da raiz (Build gate, AD-017) em todo push/PR — decisão de projeto discutida e confirmada com o usuário (AIG-43 é sistêmico, resolvido para o projeto inteiro de uma vez, não só para esta feature).
  - **Fix batch — iteração 1 do loop fix→re-verify** (5 commits, `f607bba`..`fe8ccef`): AIG-11 (claim atômico de "primeiro aviso da janela", mesmo idioma do `turnLock`/rate-limit bump/outbox claim — `guardInput.ts`/`runTurn.ts`), AIG-44+AIG-08 (5 logs estruturados + log de canal não resolvido, padrão `console.log(JSON.stringify({event...}))` de `server.ts`), AIG-48 (sticker/video reconhecidos, `caption` populado — provado via 2 novos e2e em `webhook.router.e2e.test.ts`, não via `ingest.int.test.ts` como o plano original pedia, porque `MessageType` não tem valor dedicado pra sticker/video — desvio aceito, documentado em comentário), AIG-38 (outbox real provado com uma `Message` seedada no shape exato de `createOutboundMessage`, já que `apps/ai-gateway` não depende de `apps/crm-api` — desvio aceito, documentado), AIG-41 (`AiSession` isolado entre tenants, teste novo). 739 testes passando (+11), `tsc`/`biome` limpos, verificado de forma independente pelo orquestrador (revisão própria do código de AIG-11 — o fix mais delicado — confirmada correta linha a linha).
- **In-progress**: nenhum. Fix batch (iteração 1) fechado e commitado; Verifier iteração 2 ainda não disparado nesta escrita do Handoff.
- **Next step**: disparar o **Verifier iteração 2** (sub-agente independente, fresco — author ≠ verifier) para re-checar os 5 gaps corrigidos + confirmar zero regressão no resto. Loop fix→re-verify tem teto de 3 iterações antes de escalar ao usuário (estamos na 2ª de 3). Após PASS, decisão de merge para `main` fica para o usuário pedir explicitamente (mesmo padrão de `crm-core`/`crm-web-shell`).
- **Blockers**: nenhum. Notas operacionais (histórico completo do Execute):
  1. Lotes 1, 3 e 6 sofreram interrupções de infraestrutura (sleeps/stalls/turnos prematuros/rate-limit de sessão), todas retomadas via `SendMessage` sem perda de trabalho real (cada task só commita depois do próprio gate passar); Lotes 2 exigiu um agente novo uma vez ("stopped by the user"); Lotes 4 e 5 rodaram limpos.
  2. Risco de flake pré-existente confirmado no Lote 2 (não reproduzido de forma persistente desde então, variações pontuais sob carga pesada de sandbox em lotes seguintes sempre reproduziram como pass isolado): `vitest.config.ts` já documenta a causa raiz (projects `integration`/`e2e` compartilham UMA instância de `MongoMemoryServer`, `fileParallelism:false` só serializa DENTRO de cada project). Não é regressão desta feature — gap de isolamento de test-infra pré-existente, não corrigido (fora do escopo de qualquer task; precisaria de uma decisão de projeto se for endereçado).
