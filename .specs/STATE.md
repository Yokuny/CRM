...
- **Phase / Task**: Specify → Design → Tasks (aprovadas, commit `d6df6da` em `main`) → **Execute em andamento** (Lote 2/6 completo, Lote 3/6 a disparar).
- **Completed**:
  ...
  - 586 testes passando (full suite, `pnpm vitest run`), `tsc --noEmit` limpo, `biome check .` limpo (exceto 2 itens pré-existentes fora de escopo: `noExplicitAny` em `apps/web/src/routes/_private/customers/*` de `crm-web-shell`, e um erro de formatação em `.specs/lessons.json`, arquivo de propriedade da máquina/skill — nenhum dos dois tocado por esta feature).
- **In-progress**: nenhum (Lote 2 fechado e commitado; Lote 3 ainda não disparado nesta escrita do Handoff).
- **Next step**: disparar Lote 3 (Fase 5, T18-T24 — `ingest`/`guardInput`/`contextBuild`/`runLoop`/`guardOutput`/`persist`+`dispatch`/`runTurn`), depois Lote 4 (Fase 6+7, T25-T32), Lote 5 (Fase 8+9, T33-T40), Lote 6 (Fase 10+11, T41-T47), depois o Verifier automático.
- **Blockers**: nenhum. Notas operacionais:
  1. Ambos os lotes sofreram interrupções de infraestrutura (Lote 1: 1 sleep de computador + 3 stalls de stream de 600s, todas retomadas com `SendMessage` no mesmo agente; Lote 2: 1 stall retomado, depois o agente foi encerrado ("stopped by the user", sem ação real do usuário) e não pôde ser retomado — um agente NOVO terminou T15-T17 a partir do estado em disco, sem perda de trabalho, já que cada task só commita depois do próprio gate passar).
  2. Risco de flake pré-existente confirmado: `apps/crm-api/src/routers/process.router.e2e.test.ts` falhou 1x em 3 rodadas de `pnpm vitest run` (passou isolado e nas outras 2 rodadas completas) — condição de corrida já documentada em `vitest.config.ts` (projects `integration`/`e2e` compartilham UMA instância de `MongoMemoryServer`; `fileParallelism:false` só serializa DENTRO de cada project, não ENTRE projects rodando em paralelo). O volume de testes novos desta feature deixou a corrida estatisticamente mais provável de aparecer. Não é regressão de código desta feature — é um gap de isolamento de test-infra pré-existente. Não corrigido (mudança de infra de teste fora do escopo de qualquer task; precisaria de uma decisão de projeto se for endereçado).
