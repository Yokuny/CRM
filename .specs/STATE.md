...
- **Feature**: `crm-web-shell` (feature 4 de 11) — **Execute completo, Verifier PASS, 17/17 requisitos Verified**. Ainda não mergeada em `main`, não pushada, sem PR aberto — decisão de merge fica para o usuário pedir explicitamente (mesmo padrão de `crm-core`/PR #2).
- **Phase / Task**: Specify → Design → Tasks → **Execute completo** → **Validate completo**. Nada pendente nesta feature.
- **Completed**: 29 tasks (T1-T28 + T25B, inserida em Execute — gap bloqueante de back-end achado antes da Fase 7, ver AD-030 e a nota de T25B em `tasks.md`/`design.md`) em 4 lotes de sub-agentes (Fase 1 → Fase 2 → Fases 3+4+5 → Fases 6+7+8+T25B) + 1 rodada de correção inline de 3 gaps de cobertura de teste apontados pelo Verifier (WEB-02, WEB-16, edge case de template arquivado — código já correto, só faltava a prova de teste). 2 interrupções por rate-limit de sessão (Lotes 2 e 4), ambas retomadas com sucesso via SendMessage no mesmo agente, sem perda de trabalho. 4 novas ADs gravadas nesta feature: AD-027 (stack Tailwind/ShadCN/Radix), AD-028 (DataTable sempre server-mode), AD-029 (Customer revalida contra template corrente + avança `templateVersion`), AD-030 (migração de roteamento para `createFileRoute`, achada e confirmada com o usuário a meio do Execute — feature 1 nunca tinha adotado a convenção que o próprio `CLAUDE.md` já documentava). 497 testes passando (366 antes da feature, +131), gate de build limpo, sensor de mutação sem sobreviventes, ambos os `SPEC_DEVIATION` originais (`card.tsx`, `translate.helper.ts`) fechados, zero regressão em `foundation-tenancy-auth`. 3 lições gravadas em `.specs/lessons.json` (L-014/015/016).
- **In-progress**: nenhum.
- **Next step**: usuário decide — mergear `crm-web-shell` em `main` (squash, como `crm-core`/PR #2) antes de iniciar a feature 5/11, ou seguir direto para a próxima feature sobre esta branch. Nenhuma ação automática deve ser tomada sem essa confirmação.
- **Blockers**: nenhum.
- **Uncommitted files**: nenhum — working tree limpa, tudo commitado em `feature/crm-web-shell`.
- **Branch**: `feature/crm-web-shell`, criada a partir de `main` (`28c7872`, com `crm-core`), NÃO pushada para `origin` nesta sessão (a versão pushada em `5cd5a95` é só spec.md+design.md+tasks.md confirmados, anterior ao Execute) — `git push` fica para quando o usuário pedir. `feature/crm-core`/`feature/dynamic-field-engine`/`feature/foundation-tenancy-auth` seguem intactas, históricas (conteúdo já em `main`, PR #2 `MERGED`).
