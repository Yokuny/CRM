...
- **Phase / Task**: Specify → Design → Tasks (aprovadas, commit `d6df6da` em `main`) → **Execute em andamento** (Lote 3/6 completo + fix T24B, Lote 4/6 a disparar).
- **Completed**:
  - Lote 1 (Fase 1+2, T1-T9 — `packages/db` models (`Channel`/`Conversation`/`Message`/`AiSession`) + crypto helper + `packages/contracts` schemas novos). 9 commits atômicos (`d1f36d2`..`9d5f792`) + 3 commits de correção do orquestrador: `02da05a` (nit biome em `packages/db/src/index.ts`), `5d16761` (barrel export dos 6 schemas novos em `packages/contracts/src/index.ts`), `d611a08` (glob `evals/**/*.int.test.ts` faltando no `vitest.config.ts`, adicionado ao T43, ANTES do lote).
  - Lote 2 (Fase 3+4, T10-T17 — scaffold `@crm/ai-kit`, providers Anthropic/Whisper, `TOOL_DEFINITIONS`+guard estrutural, os 4 tool executors do Anel A). 8 commits atômicos (`a6bc221`..`009fdc8`) + 1 commit de correção do orquestrador: `d1791bb` (nit biome/organize-imports em T13/T15-T17 — o Gate "full" desses tasks tinha sido rodado só com `--project integration` por uma instrução errada minha no prompt do worker, nunca com o `pnpm vitest run` completo que a tabela **Gate Check Commands** exige; corrigido rodando o gate completo antes de fechar o lote).
  - Lote 3 (Fase 5, T18-T24 — `ingest`/`guardInput`/`contextBuild`/`runLoop`/`guardOutput`/`persist`+`dispatch`/`runTurn`). 7 commits atômicos (`54a39b9`..`0ec2ba8`) + **T24B** (`04d6f80`, task nova inserida por gap encontrado pelo orquestrador, mesmo padrão do T25B/`crm-web-shell`, sem AD nova): `runTurn` retornava cedo em `mode:'human'`/`guardInput` recusado SEM NUNCA liberar o `turnLock` que `ingest` reivindicou (bug de concorrência — toda mensagem seguinte da mesma `Conversation` ficaria presa esperando um lock nunca liberado), e `guard_rejected` nunca entregava o `fixedReply` como `Message` ao cliente (spec.md exige isso explicitamente). Ambos corrigidos; os 3 testes afetados foram estendidos (não criados do zero) com asserções de `turnLock`/conteúdo da `Message`. Documentado em `tasks.md` (seção T24B completa + Phase Execution Map + Task Granularity Check atualizados). Agente do Lote 3 também sofreu 2 finalizações prematuras de turno (não "failed" — "completed" no meio de T19/T20, aparentemente por ter colocado o `pnpm vitest run` em background e esperado; corrigido instruindo rodar os gates em foreground), retomado ambas as vezes sem perda de trabalho.
  - 638 testes passando (full suite, `pnpm vitest run`), `tsc --noEmit` limpo, `biome check .` limpo (mesmos 2 itens pré-existentes fora de escopo do Lote 2, ainda não tocados).
- **In-progress**: nenhum (Lote 3 + fix T24B fechados e commitados; Lote 4 ainda não disparado nesta escrita do Handoff).
- **Next step**: disparar Lote 4 (Fase 6+7, T25-T32 — env config do `ai-gateway`, `metaClient`, middleware de assinatura do webhook, rotas do webhook, workers `outboxConsumer`/`reaper`/`idleTakeoverSweep`), depois Lote 5 (Fase 8+9, T33-T40), Lote 6 (Fase 10+11, T41-T47), depois o Verifier automático. **Lembrar o worker do Lote 4**: `runTurn` agora tem a assinatura real `runTurn(client, input: IngestInput, opts?)` com `IngestInput = {phoneNumberId, wamid, from, type, text?, mediaId?}` (`packages/ai-kit/src/ingest.ts`) — T28 (rotas do webhook) deve chamar com `phoneNumberId` cru do payload, não um `Channel` pré-resolvido (design.md tinha essa forma antiga, é um SPEC_DEVIATION documentado em `ingest.ts`).
- **Blockers**: nenhum. Notas operacionais:
  1. Todos os 3 lotes até agora sofreram interrupções de infraestrutura (Lote 1: 1 sleep + 3 stalls; Lote 2: 1 stall + 1 "stopped by the user" que exigiu um agente novo; Lote 3: 2 finalizações prematuras de turno) — todas retomadas com sucesso via `SendMessage`/agente novo, sem perda de trabalho real (cada task só commita depois do próprio gate passar).
  2. Risco de flake pré-existente confirmado no Lote 2: `apps/crm-api/src/routers/process.router.e2e.test.ts` falhou 1x em 3 rodadas de `pnpm vitest run` (passou isolado e nas outras rodadas) — condição de corrida já documentada em `vitest.config.ts` (projects `integration`/`e2e` compartilham UMA instância de `MongoMemoryServer`; `fileParallelism:false` só serializa DENTRO de cada project). Não é regressão desta feature — gap de isolamento de test-infra pré-existente, não corrigido (fora do escopo de qualquer task; precisaria de uma decisão de projeto se for endereçado).
