...
- **Phase / Task**: Specify → Design → Tasks (aprovadas, commit `d6df6da` em `main`) → **Execute em andamento** (Lote 4/6 completo, Lote 5/6 a disparar).
- **Completed**:
  ...
  - Lote 2 (Fase 3+4, T10-T17 — scaffold `@crm/ai-kit`, providers Anthropic/Whisper, `TOOL_DEFINITIONS`+guard estrutural, os 4 tool executors do Anel A). 8 commits atômicos (`a6bc221`..`009fdc8`) + 1 commit de correção do orquestrador: `d1791bb` (nit biome/organize-imports em T13/T15-T17).
  - Lote 3 (Fase 5, T18-T24 — `ingest`/`guardInput`/`contextBuild`/`runLoop`/`guardOutput`/`persist`+`dispatch`/`runTurn`). 7 commits atômicos (`54a39b9`..`0ec2ba8`) + **T24B** (`04d6f80`, task nova inserida por gap encontrado pelo orquestrador, mesmo padrão do T25B/`crm-web-shell`, sem AD nova): `runTurn` retornava cedo em `mode:'human'`/`guardInput` recusado SEM NUNCA liberar o `turnLock` que `ingest` reivindicou (bug de concorrência), e `guard_rejected` nunca entregava o `fixedReply` como `Message` ao cliente. Ambos corrigidos; os 3 testes afetados foram estendidos. Documentado em `tasks.md`.
  - Lote 4 (Fase 6+7, T25-T32 — env config do `ai-gateway`, `metaClient`, assinatura do webhook, rotas do webhook, workers `outboxConsumer`/`reaper`/`idleTakeoverSweep`, wiring). 9 commits atômicos (`bc2e3b4`..`c8add9d`, incl. 2 auto-correções do próprio worker: `67c89e0` nit de tsc, `286ff80` nit de biome — ambas achadas pelo Build gate que o worker rodou antes de fechar T32). Rodou limpo de ponta a ponta desta vez (sem stalls/interrupções) — 675 testes, verificados de novo de forma independente pelo orquestrador (`tsc`/`biome`/`vitest run` completos, baseline de biome confirmado byte-idêntico ao Lote 3). Notas aceitas sem ação (fora de escopo, corretamente adiadas pelo worker): versão da Graph API da Meta fixada em `v21.0` (decisão técnica, sem precedente no repo); `server.ts` do `ai-gateway` ganhou `StartOptions`/`StartHandle` (retrocompatível — `isMainModule` chama `start()` sem args, defaults batem com design.md); `startOutboxConsumer` ganhou `deps` injetável (mesma categoria do SPEC_DEVIATION já documentado do `runTurn`); `ai-gateway` nunca chama `syncIndexes()` diretamente — seguro porque `crm-api`'s `server.ts` já sincroniza os 4 models novos no boot compartilhado (AD-002), mas vale revisar se `ai-gateway` algum dia rodar sem `crm-api` nunca ter subido.
  - 675 testes passando (full suite, `pnpm vitest run`), `tsc --noEmit` limpo, `biome check .` limpo (mesmos 2 itens pré-existentes fora de escopo, ainda não tocados: `noExplicitAny` em `apps/web` de `crm-web-shell`, formatação de `.specs/lessons.json`).
- **In-progress**: nenhum (Lote 4 fechado e commitado; Lote 5 ainda não disparado nesta escrita do Handoff).
- **Next step**: disparar Lote 5 (Fase 8+9, T33-T40 — `apps/crm-api`: repository/service/controller/router de `Channel`, depois `Conversation` takeover/release/envio manual), depois Lote 6 (Fase 10+11, T41-T47 — wiring, isolamento entre tenants, golden set, P2 áudio), depois o Verifier automático.
- **Blockers**: nenhum. Notas operacionais:
  1. Lotes 1-3 sofreram interrupções de infraestrutura (sleeps/stalls/turnos prematuros), todas retomadas sem perda de trabalho real; Lote 4 rodou limpo, sem nenhuma interrupção.
  2. Risco de flake pré-existente confirmado no Lote 2 (não reproduzido desde então): `apps/crm-api/src/routers/process.router.e2e.test.ts` falhou 1x em 3 rodadas — condição de corrida já documentada em `vitest.config.ts` (projects `integration`/`e2e` compartilham UMA instância de `MongoMemoryServer`). Não é regressão desta feature — gap de isolamento de test-infra pré-existente, não corrigido (fora do escopo de qualquer task).
