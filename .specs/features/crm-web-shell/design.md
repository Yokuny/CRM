...
- **Interfaces**: `Button`, `Input`, `Select`/`SelectTrigger`/`SelectContent`/`SelectItem`, `Dialog`/`DialogContent`/`DialogHeader`/`DialogFooter`, `Badge`, `Card`/`CardHeader`/`CardContent`/`CardAction` (signature preserved: `asPage`, `title`), `Label`, `Tabs`/`TabsList`/`TabsTrigger`/`TabsContent`, `Form`/`FormField`/`FormItem`/`FormLabel`/`FormControl`/`FormMessage` (react-hook-form wrapper — `apps/web` already has `react-hook-form`+`@hookform/resolvers` installed, unused today), `DropdownMenu*`, `Toaster`/`toast()` (sonner), `DatePicker`, `MoneyInput`, `Switch`, `Checkbox`.
