...
export class EmptyColumnsError extends Error {}
...
export const createBoard = async (tenantId: string, data: CreateBoard): Promise<BoardRecord> => {
  if (data.columns.length === 0) throw new EmptyColumnsError('Board precisa de ao menos 1 coluna');
...
export const getBoardById = async (tenantId: string, id: string): Promise<BoardRecord> => {
  ...
  if (!board) throw new BoardNotFoundError('Board não encontrado');
  return board;
};
...
export const updateBoard = async (tenantId: string, id: string, data: UpdateBoard): Promise<BoardRecord> => {
  ...
  if (!updated) throw new BoardNotFoundError('Board não encontrado');
  return updated;
};
...
export const deleteBoard = async (tenantId: string, id: string): Promise<void> => {
  ...
  if (result.deletedCount === 0) throw new BoardNotFoundError('Board não encontrado');
...
export const removeColumn = async (tenantId: string, boardId: string, columnId: string): Promise<BoardRecord> => {
  ...
  if (!board) throw new BoardNotFoundError('Board não encontrado');
  if (board.columns.length === 1) throw new LastColumnError('Board precisa de ao menos 1 coluna');

  const hasCards = await cardRepository.existsInColumn(tenantId, boardId, columnId);
  if (hasCards) throw new ColumnNotEmptyError('A coluna precisa estar vazia para ser removida');
