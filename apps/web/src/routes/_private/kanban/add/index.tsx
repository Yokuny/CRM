        ...
        <Form {...form}>
          <form noValidate onSubmit={form.handleSubmit(onSubmit)} className="grid gap-6">
            ...
            <div className="grid gap-3 rounded-md border p-3">
              ...
              {/* Erro de array raiz (KAN-02, "board precisa de ao menos 1
                  coluna") — react-hook-form 7.44+ guarda o erro de um
                  `useFieldArray` (min/max no nível do array, não de um item)
                  em `errors.columns.root`, não em `errors.columns` direto;
                  Controller/FormField não se aplica bem a esse path
                  (useFieldArray já o registra), então lê-se formState.errors
                  direto — useForm().formState já expõe via Proxy reativo do
                  react-hook-form. */}
              {(() => {
                const columnsMessage =
                  form.formState.errors.columns?.root?.message ?? form.formState.errors.columns?.message;
                return columnsMessage ? <p className="font-medium text-destructive text-sm">{columnsMessage}</p> : null;
              })()}
            </div>
            {errorMessage && (
