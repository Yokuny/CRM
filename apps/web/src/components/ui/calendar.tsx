    ...
    <DayPicker
      ...
      classNames={{
        ...
        caption_label: cn(
          ...
          captionLayout === 'label'
            ? 'text-sm'
            : 'flex h-8 items-center gap-1 rounded-md px-4 text-sm [&>svg]:text-muted-foreground',
        ...
        today: cn('rounded-md bg-primary-blue/10 text-accent-foreground', defaultClassNames.today),
      ...
      components={{
        ...
        Chevron: ({ className, orientation, ...props }) => {
          if (orientation === 'left') {
            return <IconLeft className={cn('size-4 stroke-[1.8]', className)} {...props} />;
          }

          if (orientation === 'right') {
            return <IconRight className={cn('size-4 stroke-[1.8]', className)} {...props} />;
          }

          return <IconDown className={cn('size-4 stroke-2', className)} {...props} />;
        },
...
function CalendarDayButton({ className, day, modifiers, ...props }: ComponentProps<typeof DayButton>) {
  ...
  return (
    <Button
      ...
      className={cn(
        'flex aspect-square size-auto w-full min-w-(--cell-size) flex-col gap-1 font-normal leading-none data-[range-end=true]:rounded-md data-[range-middle=true]:rounded-sm data-[range-start=true]:rounded-md data-[range-end=true]:rounded-r-md data-[range-start=true]:rounded-l-md data-[range-end=true]:bg-primary-blue/80 data-[range-middle=true]:bg-primary-blue/20 data-[range-start=true]:bg-primary-blue/80 data-[selected-single=true]:bg-primary-blue data-[range-end=true]:text-white data-[range-middle=true]:text-accent-foreground data-[range-start=true]:text-white data-[selected-single=true]:text-white group-data-[focused=true]/day:relative group-data-[focused=true]/day:z-10 group-data-[focused=true]/day:border-ring group-data-[focused=true]/day:ring-[3px] group-data-[focused=true]/day:ring-ring/50 [&>span]:text-xs [&>span]:opacity-70',
        defaultClassNames.day,
        className,
      )}
      {...props}
