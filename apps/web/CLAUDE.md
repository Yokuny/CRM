...
- **Tela só de admin** mora em `_private/_admin/` (layout pathless, não entra na URL): o `beforeLoad` de `_admin.tsx` redireciona pra `/` quem não tem o papel `admin` (`isAdminSession`, `query/session.ts` — mesma regra do `isAdmin` do back-end). O card de navegação dela no hub também só aparece pra admin. Hoje: `/custom_fields` (templates de campos de cliente e tipos de processo).
...
| Select | `<Select items={...}>` (`@/components/ui/select.js`) — `items` é obrigatório (o tipo exige): a mesma lista `{ value, label }` que gera os `<SelectItem>`. Sem ele o Base UI mostra o valor cru no gatilho (o id do registro, um sentinel como `__all__`) |
...
| Valor de campo dinâmico em modo leitura | `<DynamicFieldValue def value>` (`@/components/dynamic-field/dynamic-field-value.js`) — título é o `label` do FieldDef, nunca o `fieldId`; o componente resolve rótulo de opção, data, moeda e sim/não |
...
**Cor também mora no primitivo, e cada token tem um papel fixo** (tabela completa, com o contraste medido de cada par, no topo de `src/index.css`): `background` (#f3f1f3) é o canvas; conteúdo (tabela, `ItemGroup`, `Panel`, input, menu, faixa do breadcrumb) sobe pra `card`; `secondary`/`accent` (menta #e5f1ee) é superfície de interação/informação (hover, selecionado, cabeçalho de tabela, ícone de hub) sempre com o `*-foreground` verde-escuro por cima; `primary` (#007a55) é ação/link/ativo; `ring` (#66af99) é foco e traço; `subtle-foreground` (#a89ea9) é só placeholder/separador — reprova como texto de leitura (2.3:1), texto secundário é `muted-foreground`. Trocar só o valor de um token raramente muda a tela: se um componente não usa o token certo pro papel, a cor nova não aparece (ou aparece no lugar errado) — ajuste o uso no primitivo e confira renderizando, nos dois temas.
...
11. **`message` da API é chave, nunca texto.** Todo `ApiResponse.message` (`@crm/contracts`, `response/index.ts`) é uma chave de `API_MESSAGE_KEYS` (`response/messages.ts`) — `respObj`/`badRespObj`/`CustomError`/`KeyedError` só aceitam `ApiMessageKey`, e detalhe técnico (campo, id, nome) vai no `detail` do erro, que só o log do servidor lê. `lib/api/client.api.ts` traduz `message` com `t()` na entrada, então a tela usa `res.message`/`error.message` direto (`toast.error(error.message)`), nunca `t(res.message)` de novo nem um `t('x_error')` fixo que descarte o motivo vindo do servidor. Chave nova na API = entrada em `API_MESSAGE_KEYS` + no dicionário (o teste-guarda trava a que faltar), seguindo as regras 1–6.

**Datas**: toda data/hora exibida passa por `@/lib/helpers/formatDate.helper.js` — `formatDate` (`PP` do date-fns — pt-BR: `1 jun 2026`), `formatTime` (`p` — `14:05`), `formatDateTime` (`1 jun 2026 · 14:05`), `formatDistanceToNow` (`há 3 dias`). O padrão vem do idioma (`Language`/`DEFAULT_LANGUAGE` em `translate.helper.ts`, hoje só `pt-BR`), nunca da tela: nada de `'dd/MM'` passado pelo chamador, `toLocaleDateString`, `Intl.DateTimeFormat` ou `{day}` cru num componente. `date-fns` só é importado nesse helper (o `<Calendar>` recebe o locale via `getDateLocale()`). Entrada `YYYY-MM-DD` é data de parede (sem conversão de fuso); qualquer outra é instante e sai no `DISPLAY_TIMEZONE`. `formatDisplayDate`/`formatDisplayTime` (`displayTime.helper.ts`) são formato de dado (search param, valor de `<input>`, chave de grade), nunca texto de tela.
